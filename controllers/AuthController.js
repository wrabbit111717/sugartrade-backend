...
const master_email = 'quanrang2021@gmail.com';
const master_password = 'raisemeup';
...
function sendVerificationEmail(email, verificationToken) {
    ...
    const msg = {
        ...
        subject: 'Register',
        html: `<p>Thanks for using our platform. You can verify your email with this link. <a href='${verificationLink}'>Verify Email</a> <br/> Thank you </p>`, // html body
      };
      sgMail.send(msg)
...
exports.forgotPassword = async (req, res, email) => {
    let foundUser = await User.findOne({ email: email });
  
    if (!foundUser) {
      res.status(400).json({errors: { email: ["The email does not match any existing user."] }});
    } else {
